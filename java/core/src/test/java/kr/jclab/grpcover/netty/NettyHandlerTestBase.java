...
public abstract class NettyHandlerTestBase<T extends GofConnectionHandler> {
  ...
  protected final void initChannel() throws Exception {
    ...
    FrameWriterLazyDecorator frameWriterLazyDecorator = new FrameWriterLazyDecorator();
    frameWriter = mock(FrameWriter.class, delegatesTo(frameWriterLazyDecorator));
    // frameReader = new DefaultHttp2FrameReader(headersDecoder);

    channel = new FakeClockSupportedChanel();
    handler = newHandler();
    frameWriterLazyDecorator.setDelegate(new DefaultGofFrameWriter(handler.connection()));
...
//  }
  ...
  protected final FrameWriter verifyWrite() {
    return verify(frameWriter);
  }

  protected final FrameWriter verifyWrite(VerificationMode verificationMode) {
  ...
  protected GofProto.Frame grpcDataFrame(int streamId, boolean endStream, byte[] content) {
    ...
    new NettyGofFrameWriter().writeData(ctx, streamId, compressionFrame, endStream,
        newPromise());
    return captureWrite(ctx);
  }

  protected final GofProto.Frame dataFrame(int streamId, boolean endStream, ByteBuf content) {
    // Need to retain the content since the frameWriter releases it.
    content.retain();

    ChannelHandlerContext ctx = newMockContext();
    new NettyGofFrameWriter().writeData(ctx, streamId, content, endStream, newPromise());
    return captureWrite(ctx);
  }

  protected final GofProto.Frame pingFrame(boolean ack, long payload) {
    ChannelHandlerContext ctx = newMockContext();
    new NettyGofFrameWriter().writePing(ctx, ack, payload, newPromise());
    return captureWrite(ctx);
  }

  protected final GofProto.Frame headersFrame(int streamId, GofProto.Header headers) {
    ChannelHandlerContext ctx = newMockContext();
    new NettyGofFrameWriter().writeHeaders(ctx, streamId, headers, false, newPromise());
    return captureWrite(ctx);
  }

  protected final GofProto.Frame goAwayFrame(int lastStreamId) {
    return goAwayFrame(lastStreamId, 0, Unpooled.EMPTY_BUFFER);
  }

  protected final GofProto.Frame goAwayFrame(int lastStreamId, int errorCode, ByteBuf data) {
    ChannelHandlerContext ctx = newMockContext();
    new NettyGofFrameWriter().writeGoAway(ctx, lastStreamId, errorCode, data, newPromise());
    return captureWrite(ctx);
  }

  protected final GofProto.Frame rstStreamFrame(int streamId, int errorCode) {
    ChannelHandlerContext ctx = newMockContext();
    new NettyGofFrameWriter().writeRstStream(ctx, streamId, errorCode, newPromise());
    return captureWrite(ctx);
  }

//  protected final ByteBuf serializeSettings(Http2Settings settings) {
//    ChannelHandlerContext ctx = newMockContext();
//    new NettyGofFrameWriter().writeSettings(ctx, settings, newPromise());
//    return captureWrite(ctx);
//  }
//
//  protected final ByteBuf windowUpdate(int streamId, int delta) {
//    ChannelHandlerContext ctx = newMockContext();
//    new NettyGofFrameWriter().writeWindowUpdate(ctx, streamId, delta, newPromise());
//    return captureWrite(ctx);
//  }
  ...
  protected final GofConnection connection() {
  ...
  protected final GofProto.Frame captureWrite(ChannelHandlerContext ctx) {
    ArgumentCaptor<GofProto.Frame> captor = ArgumentCaptor.forClass(GofProto.Frame.class);
    verify(ctx, atLeastOnce()).write(captor.capture(), any(ChannelPromise.class));
    return captor.getAllValues().get(0);
  }

  protected abstract T newHandler() throws GofException;
