...
//  }
  ...
  public void sendFrameShouldSucceed() throws Exception {
    ...
    verifyWrite().writeData(eq(ctx()), eq(STREAM_ID), eq(content()), eq(false),
  ...
  private void inboundDataShouldForwardToStreamListener(boolean endStream) throws Exception {
    ...
    GofProto.Frame frame = grpcDataFrame(STREAM_ID, endStream, contentAsArray());
    channelRead(frame);
  ...
  public void closeShouldGracefullyCloseChannel() throws Exception {
    ...
    verifyWrite().writePing(
        eq(ctx()),
        eq(false),
        ArgumentMatchers.eq(NettyServerHandler.GRACEFUL_SHUTDOWN_PING),
...
//    manualSetUp();
//    Http2Headers headers = GofProto.Header.getDefaultInstance()
//        .method(HTTP_METHOD)
//        .set(CONTENT_TYPE_HEADER, new AsciiString("application/bad", UTF_8))
//        .set(TE_HEADER, TE_TRAILERS)
//        .path(new AsciiString("/foo/bar"));
//    GofProto.Frame headersFrame = headersFrame(STREAM_ID, headers);
//    channelRead(headersFrame);
//    Http2Headers responseHeaders = GofProto.Header.getDefaultInstance()
...
//  }
  ...
  public void keepAliveManagerOnDataReceived_headersRead() throws Exception {
    manualSetUp();
    GofProto.Frame headersFrame = headersFrame(STREAM_ID, GofProto.Header.getDefaultInstance());
    channelRead(headersFrame);
  ...
  public void keepAliveEnforcer_sendingDataResetsCounters() throws Exception {
    ...
    createStream();
    GofProto.Header headers = Utils.convertServerHeaders(new Metadata());
    ChannelFuture future = enqueue(
  ...
  private void createStream() throws Exception {
    GofProto.Header headers = GofProto.Header.newBuilder()
//        .set(TE_HEADER, TE_TRAILERS)
            .setMethod("foo/bar")
            .build();
    GofProto.Frame headersFrame = headersFrame(STREAM_ID, headers);
  ...
  private GofProto.Frame emptyGrpcFrame(int streamId, boolean endStream) throws Exception {
