...
import static kr.jclab.grpcover.netty.NettyTestUtil.messageFrame;
...
public class NettyClientStreamTest extends NettyStreamTestBase<NettyClientStream> {
  ...
  public void inboundHeadersShouldCallListenerHeadersRead() throws Exception {
    stream().transportState().setId(STREAM_ID);
    GofProto.Header headers = grpcResponseHeaders();
...
//    // Set stream id to indicate it has been created
//    stream().transportState().setId(STREAM_ID);
//    GofProto.Header headers = new DefaultGofProto.Header().status(STATUS_OK).set(CONTENT_TYPE_HEADER,
//            new AsciiString("application/bad", UTF_8));
//    stream().transportState().transportHeadersReceived(headers, false);
//    GofProto.Header trailers = new DefaultGofProto.Header()
...
//  }
  ...
  @Test
  public void setGofStreamShouldNotifyReady() {
    ...
    stream().transportState().setGofStream(gofStream);
